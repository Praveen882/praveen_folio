...
set['columns'] = ('id', 'password')
set.column("#0", width=0, stretch=NO)
set.column("id", anchor=CENTER, width=80)
set.column("password", anchor=CENTER, width=80)


set.heading("#0", text="", anchor=CENTER)
set.heading("id", text="ID", anchor=CENTER)
set.heading("password", text="password", anchor=CENTER)
...
email_id = Label(Input_frame, text="ID")
email_id.grid(row=0, column=0)

full_Name = Label(Input_frame, text="password")
...
def input_record():
    ...
    set.insert(parent='', index='end', iid=count, text='',
               values=(id_entry.get(), fullname_entry.get(),))
    count += 1
